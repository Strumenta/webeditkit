...
describe('Communication.Issues', () => {
  ...
  it('getIssuesForModel on model registered, matching node id', () => {
    registerIssuesForModel("my.model", [{message:"Nothing to bad", severity:"warning",node:{regularId:"123"}}]);
    const issues = getIssuesForModel("my.model");
    expect(issues.getIssuesForNode('123')).to.eql([{message:"Nothing to bad", severity:"warning",node:{regularId:"123"}}]);
  });
  ...
  it('registerIssuesForNode on model registered, matching node id', () => {
    registerIssuesForNode({model:"my.model", id:{regularId:"123"}}, [{message:"Nothing to bad", severity:"warning",node:{regularId:"123"}}]);

    const issues = getIssuesForModel("my.model");
    expect(issues.getIssuesForNode('123')).to.eql([{message:"Nothing to bad", severity:"warning",node:{regularId:"123"}}]);

    expect(getIssuesForNode({model:"my.model", id:{regularId:"123"}})).to.eql([{message:"Nothing to bad", severity:"warning",node:{regularId:"123"}}]);
  });
  ...
  it('registerIssuesForNode for multiple nodes in same model', () => {
    registerIssuesForNode({model:"my.model", id:{regularId:"123"}}, [{message:"Nothing to bad1", severity:"warning", node:{regularId:"123"}}]);
    registerIssuesForNode({model:"my.model", id:{regularId:"124"}}, [{message:"Nothing to bad2", severity:"warning", node:{regularId:"124"}}]);
    registerIssuesForNode({model:"my.model", id:{regularId:"125"}}, [{message:"Nothing to bad3", severity:"warning", node:{regularId:"125"}}]);

    expect(getIssuesForNode({model:"my.model", id:{regularId:"123"}})).to.eql([{message:"Nothing to bad1", severity:"warning", node:{regularId:"123"}}]);
    expect(getIssuesForNode({model:"my.model", id:{regularId:"124"}})).to.eql([{message:"Nothing to bad2", severity:"warning", node:{regularId:"124"}}]);
    expect(getIssuesForNode({model:"my.model", id:{regularId:"125"}})).to.eql([{message:"Nothing to bad3", severity:"warning", node:{regularId:"125"}}]);
  });
