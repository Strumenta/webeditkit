...
import {
    childCell, editableCell,
    fixedCell, horizontalCollectionCell,
    horizontalGroupCell,
    IData,
    referenceCell,
 ...
 */
export function generateRendererFromMPSEditor(data: IData, httpCommunication: HttpCommunication) : (node: ModelNode) => Renderer {
    return (node: ModelNode) => generateRendererFromMPSEditorUsingConcept(data, httpCommunication, node.conceptName());
}

function horizontalLine() : VNode {
...
function cellToRenderer(data: IData, cellModel: ModelNode) : Renderer {
    if (cellModel.conceptName() === 'jetbrains.mps.lang.editor.CellModel_Collection') {
        const childrenRenderers : Renderer[] = cellModel.childrenByLinkName('childCellModel').map(cm => cellToRenderer(data, cm));
        if (cellModel.property('vertical') as boolean) {
            return (modelNode => verticalGroupCell(...childrenRenderers.map(r => r(modelNode))));
        } else {
        ...
        if (refData.name() === 'alias') {
            return (modelNode => fixedCell(modelNode, modelNode.alias == null ? modelNode.simpleConceptName() : modelNode.alias));
        } else {
    ...
    return (modelNode => h('span',{style:{backgroundColor: 'red'}},[`unsupported cell ${cellModel.conceptName()}`]));
}

const renderersCache : {[key:string]:Renderer} = {};

export function generateRendererFromMPSEditorUsingConcept(data: IData, httpCommunication: HttpCommunication, conceptName: string) : Renderer {
...
function generateRendererFromMPSEditorUsingConceptImpl(data: IData, httpCommunication: HttpCommunication, conceptName: string) : Renderer {
